...
describe('server input controls paddle movement', () => {
  ...
  test('directional inputs move paddle by step increments', async () => {
    ...
    const startY = s0.paddles[side];

    // Send three down steps of 0.05 each (total 0.15)
    const step = 0.05;
    a.emit('input', { roomId, dir: 'down', step });
    a.emit('input', { roomId, dir: 'down', step });
    a.emit('input', { roomId, dir: 'down', step });

    // Wait for next state
    const s1 = await waitNextState(a);
    const newY = s1.paddles[side];

    // Expect movement in the positive direction (bounded in [0,1])
    const expected = Math.max(0, Math.min(1, startY + 3 * step));
    expect(newY).toBeGreaterThanOrEqual(expected - 1e-6);

    // Now send two up steps of same size and verify it decreases
    a.emit('input', { roomId, dir: 'up', step });
    a.emit('input', { roomId, dir: 'up', step });

    const s2 = await waitNextState(a);
    const newY2 = s2.paddles[side];
    const expected2 = Math.max(0, Math.min(1, expected - 2 * step));
    expect(newY2).toBeLessThanOrEqual(expected + 1e-6);
    expect(newY2).toBeGreaterThanOrEqual(expected2 - 1e-6);
